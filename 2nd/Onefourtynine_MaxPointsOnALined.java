package 二刷;
import java.util.*;
public class Onefourtynine_MaxPointsOnALined {
	class Point{
		int x;
		int y;
		Point(){x=0;y=0;}
	...
	public int maxPoints(Point[] points){
		...
		HashMap<Double, Integer> map = new HashMap<Double, Integer>(); //how many are the same with the k
		int max = 1;
		...
		for(int i = 0; i < points.length; i++){
			...
			for(int j = i+1; j < points.length; j++){
				...
				//if the line through two points are parallel to y coordinator, then K(slop) is Integer.MAX_VALUE
